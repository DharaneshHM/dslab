...
NODE* copyNode(EMP e)
{
NODE * temp; temp= (NODE *)malloc(sizeof(NODE)); if(temp==NULL)39
{
printf("Memory cannot be allocated\n"); }
else
{
strcpy(temp->ssn,e.ssn); strcpy(temp->name, e.name); strcpy(temp->dept, e.dept); strcpy(temp->design, e.design); temp->salary=e.salary; temp->phno=e.phno;
temp->next=NULL; temp->prev=NULL; return temp;
}
}
void addrear(EMP e)
{
NODE *temp,*cur, *prev; temp=copyNode(e) ; if(first==NULL)
{
first=temp;
return;
}
...
cur->next =temp; temp->prev=prev; return ;
}
void addfront(EMP e) {
NODE *temp;40
temp=copyNode(e);
...
printf("%s\t", r->name);
printf("%s\t", r->dept);
printf("%s\t",r->design); printf("%d\t",r->salary); printf("%llu\n", r->phno);
}
void deletefront()
{
NODE *temp;
...
{
EMP e;
printf("Enter SSN: "); scanf("%s",e.ssn);
printf("Enter Name: "); scanf("%s",e.name);
printf("Enter dept: "); scanf("%s",e.dept);
printf("Enter Designation :"); scanf("%s",e.design);
printf("Enter Salary:"); scanf("%d",&e.salary); printf("Enter Phone no : "); scanf("%llu",&e.phno); return e;
}
int  main() {43
EMP e;
int i, ch, n;
